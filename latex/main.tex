...
\textbf{Scope.} We use \textbf{RoboMimic} with \textbf{low-dimensional state} on the \textbf{Lift} task in our main experiments (the public \texttt{low\_dim} dataset and simulator stack we ship against). Additional tasks (e.g., Can, Square) are natural extensions but are not part of the default configs in our repository.
...
We evaluate on \textbf{RoboMimic Lift} with \textbf{low-dimensional observations}. We report episode success, return, horizon length, realized commit-length summaries ($\mathbb{E}[m]$, etc.), \textbf{replans per episode}, and \textbf{policy forward-query counts} (chunk forwards, stochastic multi-samples, and learned head forwards where applicable). Offline metrics include validation imitation loss (and commit CE when trained).
...
\section{Current Lift Results (interim)}

Table~\ref{tab:lift_main_results} summarizes the current Lift online rollout results from the finalized run folders. At this stage, our ACT dynamic executor reaches perfect success on Lift while using substantially fewer replans than receding-horizon execution. We also report the learned-commit variant and single-step baselines implemented in this repository.
...
\toprule
Method (mode) & Success $\uparrow$ & Return $\uparrow$ & Policy queries & Replans/ep $\downarrow$ & Commit mean \\
\midrule
ACT-Strong (dynamic) & 1.00 & 60.00 & 400.0 & 86.65 & 4.56 \\
ACT-LearnedCommit (dynamic) & 1.00 & 38.35 & 400.0 & 101.25 & 3.91 \\
BC-MLP (receding\_horizon) & 0.65 & 70.70 & 400.0 & 399.0 & 1.00 \\
kNN-BC (receding\_horizon) & 0.75 & 44.40 & 400.0 & 399.0 & 1.00 \\
BeT-style (receding\_horizon) & 0.75 & 127.30 & 400.0 & 399.0 & 1.00 \\
\bottomrule
\end{tabular}
\caption{Interim Lift comparison from current run artifacts.}
\label{tab:lift_main_results}
\end{table}

Table~\ref{tab:lift_ablations_results} reports the ACT execution-mode ablations for Lift. Dynamic execution preserves success while reducing replanning compared to receding horizon, and temporal ensembling provides a strong ACT-style reference.
...
\begin{tabular}{@{}lccccc@{}}
\toprule
Mode (ACT-Strong) & Success $\uparrow$ & Return $\uparrow$ & Policy queries & Replans/ep $\downarrow$ & Commit mean \\
\midrule
Temporal ensemble (z0) & 1.00 & 76.10 & 400.0 & 0.0 & -- \\
Full chunk (z0) & 0.95 & 74.45 & 25.0 & 24.0 & 16.0 \\
Receding horizon (z0) & 0.95 & 58.15 & 400.0 & 399.0 & 1.0 \\
Dynamic (z0) & 1.00 & 60.00 & 400.0 & 86.65 & 4.56 \\
\bottomrule
\end{tabular}
\caption{Lift ablations from \texttt{runs/actdyn\_lift\_strong/eval\_ablations/z0\_default\_modes}.}
\label{tab:lift_ablations_results}
\end{table}

Across Lift multi-seed ACT-Strong runs (seeds 0/1/2), success is currently $1.00 \pm 0.00$ (mean $\pm$ std). We treat this section as interim: Can and Square parity runs are currently executing, and final cross-task claims will be updated after those logs complete.
...
\section{Conclusion}
We implemented inference-time adaptive commit-length execution for an ACT-style chunk policy, with heuristic rules, an optional learned commit head, imitation baselines (MLP BC, kNN, discrete-token BC), and shared rollout instrumentation. RT-1-scale models remain contextual citations rather than rerun baselines in this pipeline.
