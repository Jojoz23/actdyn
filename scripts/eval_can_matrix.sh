...
# Run a compact eval matrix for runs/actdyn_can_strong and snapshot outputs.
# Usage:
#   ./scripts/eval_can_matrix.sh [CHECKPOINT] [CONFIG]
#
# Defaults:
#   CHECKPOINT = runs/actdyn_can_strong/checkpoints/best.pt
#   CONFIG     = configs/can_lowdim.yaml

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

CKPT="${1:-runs/actdyn_can_strong/checkpoints/best.pt}"
CFG="${2:-configs/can_lowdim.yaml}"
TE_CFG="configs/can_lowdim_te_only.yaml"
RUN_DIR="runs/actdyn_can_strong"
