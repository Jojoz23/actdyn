...
# Resume-safe full Can+Square pipeline with Lift-level parity:
# - Main ACT train+eval
# - ACT ablation matrix (z0 / sampled / temporal-ensemble-only)
# - Baselines train+eval (bc_mlp / knn_bc / bet_style)
# - Learned-commit train+eval
# - 3-seed train+eval
#
# Usage:
#   bash /virtual/csc415user/actdyn/scripts/run_can_square_full_parity.sh

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

LOG="$ROOT_DIR/runs/overnight_can_square_full_parity_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$LOG") 2>&1
